...
export const Scene = () => {
    ...
    const Box = (box) => {
        return (
            <mesh key={box.id + '-mesh-box'} scale={box.scale} position={box.position} onClick={selectModel.bind(this, box)}>
                <boxBufferGeometry attach="geometry"/>
                <meshLambertMaterial attach="material" color={box.color} />
            </mesh>
        )
    }
    let boxesCreator = sceneData.models.map(model => <Box id={model.id} key={"model-" + model.id} scale={model.scale} position={model.position} color={model.color} /> )
